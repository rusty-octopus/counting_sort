...
fn create_test_vector<INT, RANDINT, F>(number_of_elements:usize, min_value:INT, max_value:INT, mut f:F) -> Vec<INT>
    where
        INT: Ord + Display + TryFrom<RANDINT> + Into<RANDINT> + Copy,
        F: FnMut(& mut Rand32) -> RANDINT,
        RANDINT: Copy + core::ops::Rem<Output=RANDINT> + core::ops::Add<Output=RANDINT> + Display + core::ops::Sub<Output=RANDINT> + Absolute
{
    if min_value > max_value {
        panic!("min value {} must be smaller than max_value {}", min_value, max_value);
    }
...
fn test_with_vector_i16_10k() {
    let number_of_elements = 10000;
    let mut vector = create_test_vector_signed::<i16>(number_of_elements, i16::min_value(), i16::max_value());
    let result = vector.iter().cnt_sort();
    assert!(result.is_ok());
    vector.sort();
    assert_eq!(vector, result.unwrap());
...
fn test_with_vector_u32_10k() {
    let number_of_elements = 10000;
    let mut vector = create_test_vector_unsigned::<u32>(number_of_elements, u32::from(u16::min_value()) + 1000, u32::from(u16::max_value()) + 10000);
    let result = vector.iter().cnt_sort();
    assert!(result.is_ok());
    vector.sort();
    assert_eq!(vector, result.unwrap());
...
fn test_with_vector_i32_10k() {
    let number_of_elements = 10000;
    let mut vector = create_test_vector_signed::<i32>(number_of_elements, i32::from(i16::min_value()) - 1000, i32::from(i16::max_value()) + 1000);
    let result = vector.iter().cnt_sort();
    assert!(result.is_ok());
    vector.sort();
    assert_eq!(vector, result.unwrap());
